...
@php
	...
	$displayName = $displayName !== '' ? $displayName : ($currentUser?->username ?? 'DPMBEM');

	$summaryCards = [
		['title' => 'Total Proposal', 'count' => $total ?? 0, 'icon' => 'M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5A3.375 3.375 0 0010.125 2.25H8.25m0 12.75h7.5m-7.5 3h4.5M5.625 2.25h5.603c.895 0 1.754.356 2.386.988l4.773 4.773c.632.632.988 1.49.988 2.386v8.853a2.25 2.25 0 01-2.25 2.25H5.625a2.25 2.25 0 01-2.25-2.25V4.5a2.25 2.25 0 012.25-2.25z'],
...
<div class="dashboard-shell flex flex-col gap-6">
	<div class="page-hero">
		<div class="title">
			<h1>Beranda DPMBEM</h1>
			<p>Pemantauan kegiatan dan transparansi anggaran Ormawa</p>
		</div>
	</div>

	<!-- Welcome Banner with Animations -->
	<div class="greeting-hero" style="background: linear-gradient(135deg, #eff6ff 0%, #dbeafe 100%); border: 1px solid #bfdbfe;">
		<div class="greeting-wave" aria-hidden="true">
			<span class="greeting-wave-icon">🏛️</span>
		</div>
		<div>
			<div class="greeting-title" style="color: #1e3a8a;">Halo, {{ strtolower($displayName) }}! Selamat datang di Portal Monitoring</div>
			<div class="greeting-sub" style="color: #1e40af;">Anda memiliki akses pemantauan untuk seluruh kegiatan, anggaran, dan LPJ Ormawa</div>
		</div>
	</div>
	...
	<div class="dashboard-summary-grid">
		@foreach ($summaryCards as $card)
			<div class="dashboard-card summary-card min-h-[112px] rounded-2xl bg-white px-5 py-4 shadow-sm">
				...
				<p class="mt-2 text-xs font-medium text-slate-400">Status ter-update</p>
			</div>
		@endforeach
	</div>
	...
	<div class="dashboard-card main-content-card bg-white rounded-2xl p-6 shadow-sm">
		<form method="GET" action="{{ route('admin.beranda_dpmbem') }}" class="dashboard-filter-grid mb-6">
			...
			<div class="flex flex-col gap-2">
				...
				<div class="flex gap-2">
					<input type="text" name="nama_ormawa" value="{{ request('nama_ormawa') }}" placeholder="Cari nama ormawa..." class="flex-1 rounded-xl border border-slate-200 bg-white text-sm font-medium text-slate-700 shadow-sm outline-none transition-all hover:border-slate-300 focus:border-blue-500 placeholder:text-slate-400" style="padding: 0.75rem 1.25rem;">
					<button type="submit" class="rounded-xl bg-blue-600 px-4 text-sm font-semibold text-white hover:bg-blue-700 transition" style="background-color: #2563eb;">Filter</button>
					@if(request('jenis_ormawa') || request('nama_ormawa'))
						<a href="{{ route('admin.beranda_dpmbem') }}" class="rounded-xl border border-slate-200 px-4 flex items-center text-sm font-semibold text-slate-600 hover:bg-slate-50 transition">Reset</a>
					@endif
				</div>
			</div>
		</form>
		...
		<div class="flex items-center justify-between mb-6">
			<div class="flex items-center gap-3">
				<div class="relative">
					<select class="rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-bold text-slate-700 outline-none transition-all hover:border-slate-300 focus:border-blue-500 cursor-pointer shadow-sm" style="appearance: none; -webkit-appearance: none; padding-right: 2.5rem;">
			...
			<table class="min-w-[1200px] w-full border-separate border-spacing-y-3 border-spacing-x-0 text-left text-sm text-slate-700">
				<thead class="bg-slate-100 text-slate-700">
					<tr>
						...
						<th class="px-4 py-3 font-semibold rounded-r-lg text-center">Detail</th>
					</tr>
				</thead>
				<tbody>
					@forelse ($activities as $activity)
						<tr class="group transition-colors hover:bg-slate-50">
							...
							<td class="bg-white px-4 py-4 align-middle border-y border-slate-200 group-hover:bg-slate-50 text-slate-600 leading-tight">
                                <a href="{{ route('admin.detail_kegiatan', $activity['id']) }}" class="transition-colors hover:text-blue-600 hover:underline" title="Klik untuk lihat detail">
							...
							<td class="bg-white px-4 py-4 align-middle border-y border-slate-200 group-hover:bg-slate-50 text-center">
								@if($activity['lpj_keu'])
                                    <a href="{{ asset('storage/' . $activity['lpj_keu']) }}" target="_blank" class="text-slate-400 transition hover:text-blue-600" title="Download LPJ Keuangan">
							...
							<td class="bg-white px-4 py-4 align-middle border-y border-slate-200 group-hover:bg-slate-50 text-center">
								@if($activity['lpj_keg'])
                                    <a href="{{ asset('storage/' . $activity['lpj_keg']) }}" target="_blank" class="text-slate-400 transition hover:text-blue-600" title="Download LPJ Kegiatan">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="h-5 w-5 mx-auto">
                                            <path d="M5.25 3A2.25 2.25 0 0 0 3 5.25v13.5A2.25 2.25 0 0 0 5.25 21h13.5A2.25 2.25 0 0 0 21 18.75V8.784a2.25 2.25 0 0 0-.659-1.591l-4.534-4.535A2.25 2.25 0 0 0 14.216 2.25H5.25zM12 9.75a.75.75 0 0 1 .75.75v3.19l1.22-1.22a.75.75 0 1 1 1.06 1.06l-2.5 2.5a.75.75 0 0 1-1.06 0l-2.5-2.5a.75.75 0 1 1 1.06-1.06l1.22 1.22V10.5a.75.75 0 0 1 .75-.75z" />
                                        </svg>
                                    </a>
							...
							<td class="bg-white px-4 py-4 align-middle border-y border-r border-slate-200 last:rounded-r-xl group-hover:bg-slate-50 text-center">
								<div class="flex items-center justify-center gap-3">
									<a href="{{ route('admin.detail_kegiatan', ['id' => $activity['id']]) }}" class="inline-flex h-8 w-8 items-center justify-center rounded-md bg-blue-50 text-blue-700 transition hover:bg-blue-100" title="Lihat Detail">
					...
					@empty
						<tr>
							<td colspan="11" class="text-center py-6 text-slate-400 italic bg-white border-y border-slate-200">Tidak ada data kegiatan ormawa.</td>
						</tr>
					@endforelse
				</tbody>
			</table>
