...
<div class="dashboard-shell flex flex-col gap-6">
	<div class="page-hero">
		<div class="title">
			...
			<p>Kelola dan pantau data prestasi mahasiswa yang mewakili ormawa</p>
		</div>
	</div>
	...
	<div class="dashboard-card bg-white rounded-2xl p-6 shadow-sm">
        ...
        {{-- Filter Form --}}
        <form method="GET" action="{{ route('admin.prestasi_ormawa') }}" id="prestasi-ormawa-filter-form" class="mb-6">
            <div class="flex flex-wrap gap-4 items-end">
                ...
                <div class="flex gap-2 self-end">
                    ...
                    @if(request('search') || request('tingkat'))
                        <a href="{{ route('admin.prestasi_ormawa') }}" class="rounded-xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-600 hover:bg-slate-50 transition">Reset</a>
                    @endif
                </div>
            </div>
        </form>
				...
				<tbody>
					@forelse ($prestasi as $item)
						<tr class="group transition-colors hover:bg-slate-50">
							<td class="bg-white px-4 py-4 align-middle border-y border-l border-slate-200 first:rounded-l-xl group-hover:bg-slate-50 text-slate-600">{{ $item->user->nim ?? $item->user->username ?? '-' }}</td>
							...
							<td class="bg-white px-4 py-4 align-middle border-y border-r border-slate-200 last:rounded-r-xl group-hover:bg-slate-50">
								<div class="flex items-center justify-center gap-3">
									...
									<form action="{{ route('admin.prestasi_ormawa.delete', $item->id_prestasi) }}" method="POST" style="display:inline;">
                                        @csrf @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-12 text-center text-slate-400 italic bg-white first:rounded-l-xl last:rounded-r-xl border border-slate-200">
                                Belum ada data prestasi ormawa.
                            </td>
                        </tr>
					@endforelse
				</tbody>
    ...
    {{-- Daftar Pengajuan Proposal Kegiatan Ormawa --}}
    <div class="dashboard-card bg-white rounded-2xl p-6 shadow-sm mt-8">
        <div class="flex items-center justify-between mb-6">
            <div class="title">
                <h2 class="text-lg font-bold text-slate-800">Daftar Pengajuan Proposal Kegiatan Ormawa</h2>
                <p class="text-sm text-slate-500">Pantau pengajuan dana kegiatan lomba yang diajukan oleh ormawa</p>
            </div>
        </div>
        ...
        <div class="dashboard-table-wrap">
			<table class="min-w-full w-full border-separate border-spacing-y-3 border-spacing-x-0 text-left text-sm text-slate-700">
				...
				<tbody>
					@forelse ($ormawProposals as $prop)
						<tr class="group transition-colors hover:bg-slate-50">
							...
							<td class="bg-white px-4 py-4 align-middle border-y border-r border-slate-200 last:rounded-r-xl group-hover:bg-slate-50 text-center">
								<a href="{{ route('admin.form_verifikasi', ['id' => $prop->id_proposal]) }}?type=ormawa" class="inline-flex h-8 w-8 items-center justify-center rounded-md bg-sky-50 text-sky-700 transition hover:bg-sky-100" title="Verifikasi Proposal">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="h-4 w-4">
					...
					@empty
                        <tr>
                            <td colspan="6" class="px-4 py-12 text-center text-slate-400 italic bg-white first:rounded-l-xl last:rounded-r-xl border border-slate-200">
                                Belum ada pengajuan proposal kegiatan ormawa.
                            </td>
                        </tr>
					@endforelse
				</tbody>
