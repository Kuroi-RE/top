            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
                @foreach($templates as $item)
                    <div class="p-5 rounded-2xl border border-gray-200 bg-white hover:border-red-200 transition shadow-sm hover:shadow-md flex items-center justify-between gap-4">
                        ...
                        <a href="{{ route('prestasi.template_download', $item['id_template']) }}" target="_blank" class="px-4 py-2 rounded-full text-xs font-bold bg-red-600 hover:bg-red-700 text-white transition shrink-0 shadow-sm flex items-center gap-1.5">
