...
<div class="min-h-screen bg-gray-100 flex justify-center px-4 py-8">
    ...
    @php
        $fields = [
            ...
            ['label' => 'Besar Ajuan', 'name' => 'besar_ajuan', 'type' => 'text'],
    ...
    <form method="POST" enctype="multipart/form-data" class="mt-4 space-y-6 sm:mt-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-[150px_1fr] items-start gap-3 md:gap-6">
            ...
            <div class="flex flex-wrap items-center gap-5 pt-1">
                <label class="inline-flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                    <input type="radio" name="honor_pelatih" value="Ya"
                        class="h-5 w-5 border-gray-400 accent-red-600">
                    <span>Ya</span>
                </label>
                <label class="inline-flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
                    <input type="radio" name="honor_pelatih" value="Tidak"
                        class="h-5 w-5 border-gray-400 accent-red-600">
                    <span>Tidak</span>
                </label>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Intercept actual submit to avoid sending POST (route may not accept POST)
    ...
    function showClientSuccess(message) {
        ...
        // Redirect to organisasi index (proposal listing) after popup finishes
        setTimeout(function() { window.location.href = '{{ route("organisasi.index") }}'; }, AUTO_HIDE_MS + FADE_MS + 220);
    }
});
</script>
@endpush